...
public class withdrawAmt extends HttpServlet {
     ...
     protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       double  withdraw_amt = Double.parseDouble(request.getParameter("withdraw_amt"));
       String Particulars = request.getParameter("Particulars");
        try {
            ...
            double balance = rs.getDouble("balance");
              double updatedbalance = balance - withdraw_amt;
                
              PreparedStatement stmt1 = conn.prepareStatement("insert into transaction (date,acc_no,withdraw,particulers,balance) values (?,?,?,?,?)");
              currentDate date = new currentDate();
              
              stmt1.setString(1, date.date());
              stmt1.setString(2, acc_no);
              stmt1.setDouble(3, withdraw_amt);
          ...
          int j = stmt12.executeUpdate();
            ...
            if(i>0 && j>0){
                ...
                request.getSession().setAttribute("model","Withdraw amount successfully");
